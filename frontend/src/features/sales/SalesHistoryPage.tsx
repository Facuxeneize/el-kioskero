  ...
  return (
    <div className="page">
      ...
      <section className="panel">
        ...
        {sales.isLoading ? <div className="empty">Cargando ventas…</div> : sales.isError ? <div className="alert panel-alert">{sales.error.message}</div> : sales.data?.items.length ? <div className="table-wrap"><table><thead><tr><th>Número</th><th>Fecha y hora</th><th>Productos</th><th>Unidades</th><th>Total</th><th>Estado</th><th></th></tr></thead><tbody>{sales.data.items.map((sale) => <tr key={sale.id}><td><strong>#{sale.saleNumber}</strong></td><td>{formatDateTime(sale.createdAt)}</td><td>{sale.items.length}</td><td>{sale.totalUnits}</td><td><strong>{formatMoney(sale.total)}</strong></td><td><span className={sale.status === 'COMPLETED' ? 'pill success' : 'pill neutral'}>{sale.status === 'COMPLETED' ? 'Completada' : 'Anulada'}</span></td><td><Link className="table-link" to={`/ventas/${sale.id}`}>Ver detalle →</Link></td></tr>)}</tbody></table></div> : <div className="empty"><strong>No hay ventas para estos filtros</strong><p>Las operaciones confirmadas aparecerán acá.</p></div>}
      </section>
    </div>
  )
}
