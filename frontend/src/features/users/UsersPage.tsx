...
export function UsersPage() {
  ...
  const save = useMutation({
    mutationFn: () => apiRequest<ManagedUser>(`/users/${editing!.id}`, {
  ...
  function openEdit(user: ManagedUser) {
    setEditing(user)
    setForm({ name: user.name, kioskName: user.kioskName, username: user.username, email: user.email, role: user.role, isActive: user.isActive })
    setMessage('')
    save.reset()
  }
